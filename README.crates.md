...
`ResizeConfig::validate` caps output at **120 MP** by default (`max_output_pixels`, `Some(120_000_000)` — large enough for 108 MP phone photos) and rejects zero/NaN/∞/out-of-range configs, so untrusted *target* dimensions can't trigger a runaway allocation. Lift or tighten the cap via the `max_output_pixels` field. There's no built-in cancellation token — cancel cooperatively at the `push_row` / `next_output_row` boundary.
...
- Hardened against adversarial inputs: weight-table allocation is bounded, NaN/∞/out-of-range config fields are rejected, and all buffer sizing uses checked arithmetic — a few-byte container header can't trigger a multi-GB allocation
...
zenresize is single-threaded per call by design — it computes one resize on the calling
thread, leaving parallelism to the caller (resize N images across a thread pool, or run
the streaming API inside a pipeline stage). Benchmarks therefore compare single-thread vs
single-thread; a library with internal threading would need its pool disabled for an
apples-to-apples per-call comparison (which `paired_bench` does).
...
Dual-licensed: [AGPL-3.0](https://github.com/imazen/zenresize/blob/main/LICENSE-AGPL3) or [commercial](https://github.com/imazen/zenresize/blob/main/LICENSE-COMMERCIAL).
...
See [LICENSE-COMMERCIAL](https://github.com/imazen/zenresize/blob/main/LICENSE-COMMERCIAL) for details.
